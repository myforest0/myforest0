...
urlname: k8s
title: K8s笔记
updated: '2024-01-13 16:30:19'
date: '2024-01-13 16:25:22'
status: 已发布
catalog:
  - 后端
---
...
## 互联网项目部署方式历史发展
### 应用程序部署方式
![Untitled.png](/images/d71f41bdea53d974e5304a01d1aab82e.png)
- 传统部署时代
	- 早期的时候，在物理服务器上运行应用程序
		- 缺点
...
- kubernetes常⻅概念
	- Master
		...
		- Pod是一组容器, 在K8S中，最小的单位是Pod，一个Pod可以包含多个容器，但通常情况下我们在每个Pod中仅使用一个容器
			...
			![Untitled.png](/images/b095e73e04e04ef17247f4db28dbb446.png)	- 分类
		...
		- K8S提供了一种机制来为Pod进行分类，那就是Label(标签)，同一类pod会拥有相同的标签
			...
			- 未使用前，分散难管理，如果需要部署不同版本的应用到不同的环境中，难操作
				![Untitled.png](/images/cb63a7484dc5a5809705bbe5896ba0d6.png)	- 为Pod打上不同标签，使用Label组织的Pod，轻松管理
				![Untitled.png](/images/5344bccf3e11ce3e1c5e0d0833778a33.png)	- Label选择器
...
- kubernetesK8S整体架构
	![Untitled.png](/images/348fef939eb6d2622e59db9707d99c1a.png)	- 也是Client-Server模型
		![Untitled.png](/images/9308d0df7d667b2cf893dc8823cda51d.png)	- 控制节点Master-Node，负责集群的管理 (Master也可以多个，保证高可用)
...
- k8s集群类型有多种
	- 单master-Node集群
		- 一主多从，推荐测试环境
			![Untitled.png](/images/5897c4c69eeb53051b589b51cb0223fd.png)	- 服务器要求：服务器要求至少2台2核4G以上的云服务器
			- 多master-Node集群
		- 多主多从(高可用集群)，推荐生产环境使用
			![Untitled.png](/images/1dc85d936f1b7e239216017c4361aeb4.png)	- 服务器要求：服务器要求至少4台2核4G以上的云服务器
			- 单节点k8s集群
		- 单一个节点
...
- 查看节点信息
	```shell
	kubectl get node
	```	![Untitled.png](/images/d61ab5b31285a52928973ef4b421781b.png)
- 查看内部组件
	```shell
	kubectl get pod -A
	```	![Untitled.png](/images/ea27e00b2a45c64f37d3256689fe2b07.png)
- 部署第一个K8S应用-Nginx，并通过公网ip访问
	- 创建deployment(Pod控制器的一种)
		...
		```	![Untitled.png](/images/63108f9a33992eea965cdc90b4ea7733.png)	- 暴露80端口，创建service
		```shell
		kubectl expose deployment xdclass-nginx --port=80 --type=NodePort
		```	- 查看端口映射
		```shell
		kubectl get pod,svc
		```	![Untitled.png](/images/a38b8d16ec36f6ed898d4f69f9d12d25.png)	- 在浏览器访问, 工作节点开放端口31512，访问工作节点ip: 31512(网络安全组记得开放31512端口)
		- master、node节点访问这个 开放的端口都可以
		
- 注意
	- Kubeadm部署，暴露端口对外服务，会随机选端口，默认范围是30000~32767，可以修改指定
		![Untitled.png](/images/5fe3fc226e7970946b50dff19d0dc4d3.png)
